...
export function BookingForm({ onClose, carPrice = 0 }: BookingFormProps) {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (values.paymentMethod === "upi") {
      const upiUrl = `upi://pay?pa=carsify@upi&pn=Carsify&am=${totalPrice}&cu=INR&tn=Car%20Rental%20Payment`;
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        {form.watch("paymentMethod") === "upi" && totalPrice && (
          <div className="p-4 bg-secondary/20 rounded-lg text-center">
            ...
            <div className="w-48 h-48 mx-auto bg-white p-4 rounded-lg">
              ...
              <p className="text-black text-xs mt-2">Amount: ₹{totalPrice}</p>
            </div>
          </div>
        )}
